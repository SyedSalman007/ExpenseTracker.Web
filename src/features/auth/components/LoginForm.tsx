...
export function LoginForm() {
  ...
  return (
    <>
      {/* Mobile layout */}
      <div className="w-full max-w-96 md:hidden">
        <div className="mb-xl text-center">
          <div className="mb-md inline-flex h-14 w-14 items-center justify-center rounded-xl bg-primary text-on-primary shadow-lg">
          ...
          <p className="mt-xs text-sm text-on-surface-variant">
            Manage your financial wellness with ease.
          </p>
        </div>

        <div className="glass-card rounded-xl border border-outline-variant p-lg shadow-lg">
          <form onSubmit={handleSubmit} className="space-y-md">
            <div className="space-y-xs">
              <label
                htmlFor="email-mobile"
                className="block px-xs text-xs font-semibold uppercase tracking-wider text-on-surface-variant"
              >
                Email Address
              </label>
              <div className="relative flex items-center rounded-lg border border-outline-variant bg-surface-container-lowest transition-all focus-within:border-primary">
                <Icon name="mail" className="absolute left-3 !text-xl text-outline" />
                <input
                  id="email-mobile"
                  type="email"
                  ...
                  className="w-full border-none bg-transparent py-3 pl-10 pr-3 text-sm focus:ring-0"
                  required
                />
              </div>
            </div>
            ...
            <div className="space-y-xs">
              ...
              <div className="relative flex items-center rounded-lg border border-outline-variant bg-surface-container-lowest transition-all focus-within:border-primary">
                <Icon name="lock" className="absolute left-3 !text-xl text-outline" />
                <input
                  id="password-mobile"
                  ...
                  className="w-full border-none bg-transparent py-3 pl-10 pr-12 text-sm focus:ring-0"
                  required
                />
                <button
                  type="button"
                  onClick={() => setShowPassword((v) => !v)}
                  className="absolute right-3 text-outline transition-colors hover:text-on-surface-variant"
                >
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="mt-lg flex w-full items-center justify-center gap-base rounded-lg bg-primary py-3.5 text-lg font-semibold text-on-primary shadow-md transition-all hover:bg-primary-container hover:shadow-lg active:scale-[0.98] disabled:opacity-60"
            >
              {isLoading ? "Signing in..." : "Sign In"}
              {!isLoading && <Icon name="arrow_forward" className="!text-xl" />}
            </button>
          </form>
          ...
          <div className="relative my-lg">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="bg-white px-2 font-semibold uppercase tracking-wider text-on-surface-variant">
                Or continue with
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-sm">
            <button
              type="button"
              className="flex items-center justify-center gap-sm rounded-lg border border-outline-variant bg-surface-container-lowest py-2.5 text-sm text-on-surface transition-colors hover:bg-surface-container active:scale-95"
            >
            ...
            >
              <svg className="h-5 w-5" viewBox="0 0 24 24">
                <path d="M17.05 20.28c-.98.95-2.05.88-3.08.4-1.09-.5-2.08-.48-3.24 0-1.44.62-2.2.44-3.06-.4C4.22 16.8 4.22 10.62 7.7 10.3c1.08.06 1.83.63 2.5.63.66 0 1.77-.73 3.12-.58 1.4.15 2.45.75 3.02 1.6-2.8 1.55-2.35 5.2.45 6.34-.58 1.44-1.34 2.8-2.74 4zM12.03 9.4c-.1-.02-.15-.02-.2 0-2.33-.18-3.05-2.73-2.93-4.52 2.22.1 3.53 2.13 3.13 4.52z" fill="currentColor" />
              </svg>
              Apple
            </button>
          </div>
          ...
          <div className="mt-xl flex items-center gap-md rounded-xl border border-outline-variant/50 bg-surface-container-high/50 p-md">
      ...
      {/* Desktop layout */}
      <div className="glass-card hidden w-full max-w-[1000px] overflow-hidden rounded-xl md:block">
